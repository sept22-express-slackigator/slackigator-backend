...
module.exports = Router()
  ...
  .get('/', async (req, res, next) => {
    ...
    const payloadEvents = events.map((event) => {
      ...
      payload.blocks[1].text.text = event.title;
      payload.blocks[2].text.text = `<${event.detail_url}|Global PDX event page>`;
      payload.blocks[3].fields[0].text = `*When:*\n${event.date}`;
      ...
      try {
        ...
        res.json(payloadEvents);
      } catch (e) {
        next(e);
      }
    });

    // console.log('mapped array of events', payloadEvents[0].json());
  });
